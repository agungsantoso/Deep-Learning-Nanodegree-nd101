    ...
    for each in classes:
        ...
        for ii, file in enumerate(files, 1):
            ...
            if ii % batch_size == 0 or ii == len(files):
                ...
                codes_batch = sess.run(fetches=vgg.relu6, feed_dict={input_ : images} )
...
train_idx, test_idx = next(sss.split(codes, labels_vecs))
split = len(test_idx) // 2
val_idx, test_idx = test_idx[split:], test_idx[:split]
...
logits = tf.contrib.layers.fully_connected(inputs=fc, num_outputs=labels_vecs.shape[1], activation_fn=None)
