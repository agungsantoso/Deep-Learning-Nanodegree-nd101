...
def create_lookup_tables(text):
    ...
    """
    all_words = set(text)
    vocab_to_int = {word : idx for idx, word in enumerate(all_words)}
    int_to_vocab = {idx : word for idx, word in enumerate(all_words)}
...
def get_init_cell(batch_size, rnn_size, lstm_layers=2):
    ...
    cell = tf.contrib.rnn.MultiRNNCell(cells= [lstm] * lstm_layers)
...
def build_nn(cell, rnn_size, input_data, vocab_size):
    ...
    logits = tf.contrib.layers.fully_connected(inputs=output,
...
def get_batches(int_text, batch_size, seq_length):
    ...
    x_end_idx = n_batches * batch_size * seq_length
    y_end_idx = x_end_idx + 1
    x = np.array(int_text[:x_end_idx])
    y = np.array(int_text[1:y_end_idx])
    x = np.reshape(x, (n_batches, batch_size, seq_length))
    y = np.reshape(y, (n_batches, batch_size, seq_length))
    #x = np.reshape(x, (-1, seq_length))
    #y = np.reshape(y, (-1, seq_length))
    #x = [[x[b_num + b*n_batches] for b in range(batch_size)] for b_num in range(n_batches)]
    #y = [[y[b_num + b*n_batches] for b in range(batch_size)] for b_num in range(n_batches)]
